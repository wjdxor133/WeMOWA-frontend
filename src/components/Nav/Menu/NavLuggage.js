import React, { Component } from "react";
import "./NavLuggage.scss";

class NavLuggage extends Component {
  render() {
    return (
      <div className="NavLuggage">
        <ul>
          <li className="li1">
            <span>COLLECTION</span>
            <div className="listItem1 ">
              <div className="COLLECTION-left-list">
                <a className="COLLECTION-item1" href="#">
                  <img
                    src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dwcdbcebe5/images/category_megamenu/1_ORIGINAL.png"
                    alt="ORIGINAL-img"
                  ></img>
                  <div className="COLLECTION-name">
                    <p>ORIGINAL</p>
                    <p>A travel icon</p>
                  </div>
                </a>
                <a className="COLLECTION-item2" href="#">
                  <img
                    src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw9cef6835/images/category_megamenu/6_CLASSIC.png"
                    alt="CLASSIC-img"
                  ></img>
                  <div className="COLLECTION-name">
                    <p>CLASSIC</p>
                    <p>Heritage</p>
                    <p>inspired</p>
                  </div>
                </a>
                <a className="COLLECTION-item3" href="#">
                  <img
                    src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw261846fd/images/category_megamenu/Hybrid_White_megamenu.png"
                    alt="HYBRID-img"
                  ></img>
                  <div className="COLLECTION-name">
                    <p>HYBRID</p>
                    <p>Best of both</p>
                    <p>worids</p>
                  </div>
                </a>
              </div>
              <div className="COLLECTION-right-list">
                <a className="COLLECTION-item4" href="#">
                  <img
                    src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw261846fd/images/category_megamenu/Hybrid_White_megamenu.png"
                    alt="ESSENTIAL-img"
                  ></img>
                  <div className="COLLECTION-name">
                    <p>ESSENTIAL</p>
                    <p>Fiexible by</p>
                    <p>design</p>
                  </div>
                </a>
                <a className="COLLECTION-item5" href="#">
                  <img
                    src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw261846fd/images/category_megamenu/Hybrid_White_megamenu.png"
                    alt=">ESSENTIALLITE-img"
                  ></img>
                  <div className="COLLECTION-name">
                    <p>ESSENTIAL LITE</p>
                    <p>Our lightest</p>
                    <p>suitcase</p>
                  </div>
                </a>
                <a className="COLLECTION-item6" href="#">
                  <img
                    src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw261846fd/images/category_megamenu/Hybrid_White_megamenu.png"
                    alt="ESSENTIAL-img"
                  ></img>
                  <div className="COLLECTION-name">
                    <p>ESSENTIAL</p>
                    <p>SLEEVE</p>
                    <p>Business trip ready</p>
                  </div>
                </a>
              </div>
            </div>
            <a className="seeAllCollection" href="#">
              See All Collection
            </a>
          </li>
          <li className="li2">
            <span>COLOUR</span>
            <div className="color">
              <div className="color-left">
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Black"
                    ></img>
                    <p>Black</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Coral"
                    ></img>
                    <p>Coral</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Red"
                    ></img>
                    <p>Red</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Slate"
                    ></img>
                    <p>Slate</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="White"
                    ></img>
                    <p>White</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Glacier"
                    ></img>
                    <p>Glacier</p>
                  </div>
                </a>
              </div>
              <div className="color-right">
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Blue"
                    ></img>
                    <p>Blue</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Green"
                    ></img>
                    <p>Green</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Saffron"
                    ></img>
                    <p>Saffron</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      ali="Silver"
                    ></img>
                    <p>Silver</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Berry"
                    ></img>
                    <p>Berry</p>
                  </div>
                </a>
                <a href="#">
                  <div className="color-row">
                    <img
                      src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw76e8640c/images/16/megamenu/black_cat_menu.png"
                      alt="Titanium"
                    ></img>
                    <p>Titanium</p>
                  </div>
                </a>
              </div>
            </div>
          </li>
          <li className="li3">
            <span>SIZE</span>
            <div className="size-top">
              <a className="size-cabin" href="#">
                <img
                  src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dwed0b1d76/images/category_megamenu/Orig_Cabin_PF-FINAL.png"
                  alt="Cabin"
                ></img>
                <p>Cabin</p>
              </a>
              <a className="size-checkIn" href="#">
                <img
                  src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dw0df46682/images/category_megamenu/Orig_CheckinM_PF-FINAL.png"
                  alt="Check-In"
                ></img>
                <p>Check-In</p>
              </a>
              <a className="size-trunk" href="#">
                <img
                  src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dwd3368939/Orig_Trunk_PF-FINAL (002).png"
                  alt="Trunk"
                ></img>
                <p>Trunk</p>
              </a>
            </div>
            <div className="material-bottom">
              <p>MATERIAL</p>
              <div className="material-color">
                <div className="materialColor-1">
                  <img
                    src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dwe48a2c1f/images/category_megamenu/aluminium_cat_menu.png"
                    alt="Alumininm Cases"
                  ></img>
                  <p>Alumininm Cases</p>
                </div>
                <div className="materialColor-2">
                  <img
                    src="https://www.rimowa.com/on/demandware.static/-/Sites-rimowa-storefront-final/default/dwf03b7be3/images/category_megamenu/polycarbonate_cat_menu.png"
                    alt="Polycarbonate Cases"
                  ></img>
                  <p>Polycarbonate Cases</p>
                </div>
              </div>
            </div>
          </li>
          <li className="li4">
            <span>TRAVEL TYPE</span>
            <div className="short-trip">
              <img
                src="https://www.rimowa.com/on/demandware.static/-/Library-Sites-RimowaSharedLibrary/default/dwdacb5f82/images/50/Megamenu/WeekEnd@2x.jpg"
                alt="SHORT TRIP"
              ></img>
              <div className="shortTrip-content">
                <p>SHORT TRIP</p>
                <p>Travelling light for 2-3</p>
                <p>days</p>
              </div>
            </div>
            <div className="business-trip">
              <img
                src="https://www.rimowa.com/on/demandware.static/-/Library-Sites-RimowaSharedLibrary/default/dw8adf39f4/images/50/Megamenu/Business@2x.jpg"
                alt="BUSINESS TRIP"
              ></img>
              <div className="businessTrip-content">
                <p>BUSINESS TRIP</p>
                <p>Professional essentials for</p>
                <p>business travel</p>
              </div>
            </div>
            <div className="long-haul">
              <img
                src="https://www.rimowa.com/on/demandware.static/-/Library-Sites-RimowaSharedLibrary/default/dw6a9f4295/images/50/Megamenu/LongHaul@2x.jpg"
                alt="LONG HAUL"
              ></img>
              <div className="longHaul-content">
                <span>LONG HAUL</span>
                <p>Extended getaways of a</p>
                <p>week or more</p>
              </div>
            </div>
          </li>
        </ul>
      </div>
    );
  }
}

export default NavLuggage;
